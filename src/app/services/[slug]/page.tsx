...
export default function ServiceDetailPage() {
  ...
  const [category, setCategory] = useState<CategoryService | null>(null);
  ...
  const [prevService, setPrevService] = useState<Service | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const serviceData = await getServiceByUrl(slug);
        if (!serviceData) {
          setNotFound(true);
          return;
        }
        ...
        setViewCount((serviceData.view_count ?? 0) + 1);
        if (serviceData.id)
          incrementServiceView(serviceData.id).then(setViewCount);
        if (serviceData.category_id)
          getCategoryServiceById(serviceData.category_id).then(setCategory);
        getLatestBlogs(5).then(setLatestBlogs);

        if (serviceData.created_at && serviceData.id) {
          getAdjacentServices(serviceData.created_at, serviceData.id).then(
            ({ prev, next }) => {
              setPrevService(prev);
              setNextService(next);
            },
        ...
        if (serviceData.category_id && serviceData.id) {
          getAllServices({
            page: 1,
            limit: 5,
  ...
  useEffect(() => {
    if (!service) return;
    const rawHtml = service.content[lang] || service.content.vi || "";
  ...
  const categoryName = category ? category.name[lang] || category.name.vi : "";
  const authorName = service.author_info?.name ?? null;
  ...
  // FAQ data — sort theo order
  const faqs: FAQ[] = service.faqs
    ? [...service.faqs].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    : [];
  ...
  return (
    <div className="w-full bg-white">
      <style jsx global>{`
        ...
        .service-content div[data-type="highlight-block"] {
        ...
        .service-content div[data-type="highlight-block"] ul {
          list-style: none;
          margin: 0.25rem 0 0 0;
          padding-left: 0;
        }
        .service-content div[data-type="highlight-block"] ul li {
          position: relative;
          padding-left: 1.5rem;
          margin-bottom: 0.4rem;
          line-height: 1.7;
        }
        .service-content div[data-type="highlight-block"] ul li::before {
          content: "•";
          position: absolute;
          left: 0;
          color: #06446a;
        ...
        .service-content div[data-type="highlight-block"] ol {
          counter-reset: item;
          list-style: none;
          margin: 0.25rem 0 0 0;
          padding-left: 0;
        }
        .service-content div[data-type="highlight-block"] ol li {
          position: relative;
          padding-left: 2rem;
          margin-bottom: 0.4rem;
        ...
        .service-content div[data-type="highlight-block"] ol li::before {
        ...
        .service-content div[data-type="highlight-block"] p {
          margin-bottom: 0.4rem;
          text-align: left;
        }
        .service-content div[data-type="highlight-block"] p:last-child {
          margin-bottom: 0;
        }
        .service-content div[data-type="highlight-block"] strong,
        .service-content div[data-type="highlight-block"] b {
          font-weight: 700;
        }
        .service-content div[data-type="highlight-block"] a {
          color: #06446a;
          text-decoration: underline;
          text-decoration-color: #e7bf64;
          text-underline-offset: 3px;
        }
        .service-content div[data-type="highlight-block"] a:hover {
          color: #e7bf64;
        }
        .service-content div[data-type="highlight-block"] h1,
        .service-content div[data-type="highlight-block"] h2,
        .service-content div[data-type="highlight-block"] h3 {
          color: #06446a;
          margin-top: 0;
      ...
      <div className="w-full max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <main className="lg:col-span-2">
            ...
            {/* FAQ Accordion */}
            {faqs.length > 0 && <FaqAccordion faqs={faqs} lang={lang} />}
...
/* ===== FAQ Accordion ===== */
function FaqAccordion({ faqs, lang }: { faqs: FAQ[]; lang: Lang }) {
  ...
  return (
    <div className="mt-10">
      ...
      <div className="divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden">
        {faqs.map((faq, index) => {
          const question =
            faq.question[lang] || faq.question.vi || faq.question.en || "";
          const answer =
            faq.answer[lang] || faq.answer.vi || faq.answer.en || "";
          ...
          return (
            <div key={index} className="bg-white">
              <button
                ...
                className="w-full flex items-center justify-between gap-3 px-5 py-4 text-left transition-colors hover:bg-gray-50"
                aria-expanded={isOpen}
              >
                <span
                  className="text-[15px] font-semibold leading-snug flex-1"
                  style={{ color: isOpen ? PRIMARY : "#1a1a1a" }}
                >
                  {question}
                </span>
                <span
                  ...
                  style={{
                    backgroundColor: isOpen ? PRIMARY : "#f3f4f6",
